...
class Autoencoder(nn.Module):
	...
	def __init__(self, input_size, expected_size, dropout):
		...
		encode_list = [ nn.Linear(input_size, nearest_2_power) ]
		size = nearest_2_power
		while size > (2 * expected_size):
			new_size = size // 2
			enc = nn.Linear(in_features=size, out_features=new_size)
			encode_list.extend([nn.ReLU(True),nn.Dropout(dropout), enc])
			size = new_size
		...
		while size < (input_size // 2):
			...
			decode_list.extend([nn.ReLU(True),nn.Dropout(dropout),dec])
			size = new_size
		if size < input_size:
...
def train(model, x, epoch=25):
	...
	for i in range(epoch):
		...
		for data in loader:
			optimizer.zero_grad()
			output = model(data[0])
			loss = loss_fn(output, data[0])
			loss.backward()
			optimizer.step()
			running_loss += loss.item()
		epoch_loss = running_loss / len(x)
		print ("Finished epoch {}/{}, loss: {}".format(i+1, epoch,
			epoch_loss))
	model.eval()
	return model

def evaluate(model, x, metrics):
	model.eval()
	x = torch.from_numpy(x.to_numpy()).to(DEVICE).float()
	dataset = TensorDataset(x)
	loader = DataLoader(dataset, batch_size=BATCH_SIZE)
